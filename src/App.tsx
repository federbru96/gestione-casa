...
export default function App() {
  ...
  const handleApplyUtility = async (e: React.FormEvent) => {
    ...
    let fileName = paymentToUpdate.utility_file_url;
    const fileInput = (
      document.getElementById('pdf-file-input') as HTMLInputElement
    )?.files?.[0];

    if (fileInput) {
      const filePath = `bolletta_${monthId}_${Date.now()}.pdf`;
      ...
      if (uploadError) {
        alert(
          'Errore durante il caricamento del file PDF: ' + uploadError.message
        );
        return;
      }
    ...
    if (!error) {
      ...
      alert('Bolletta caricata e aggiunta con successo!');
      fetchData();
      setBillForm({
        type: 'Luce',
        billAmount: '',
  ...
  const togglePaymentStatus = async (id: number, currentStatus: string) => {
    ...
    const payDate =
      newStatus === 'Pagato' ? new Date().toISOString().split('T')[0] : null;
  ...
  const generatePDF = (payment: any) => {
    ...
    doc.text(
      `Inquilino: ${contract?.tenant_name} (${contract?.tenant_cf})`,
      20,
      58
    );
    ...
    doc.text(
      "Imposta di bollo da 2,00€ assolta sull'originale se dovuta.",
      20,
      152
    );

    doc.save(
      `Ricevuta_${contract?.tenant_name.replace(
        ' ',
        '_'
      )}_${payment.month.replace(' ', '_')}.pdf`
    );
  };
            ...
            >
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      Scadenza: il {contract?.due_day} del mese
                    </p>
                  </div>
                  <span
                    style={{
              ...
              >
                ...
                >
                  ...
                  <h2 style={{ fontSize: '18px', margin: 0 }}>
                    Carica Nuova Bolletta PDF
                  </h2>
                </div>

                <form
                  onSubmit={handleApplyUtility}
                  style={{
                    display: 'flex',
                    flexDirection: 'column',
                    gap: '10px',
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 8px' }}>
                        {p.utility_file_url ? (
                          ...
                          >
                            <Paperclip size={12} /> {p.utility_file_url}
                          </span>
                        ) : (
        ...
        {viewMode === 'tenant' && (
          ...
          >
            ...
            >
              ...
              <div>
                <h2 style={{ fontSize: '18px', margin: 0, color: '#2d3748' }}>
                  Benvenuta, {contract?.tenant_name}
                </h2>
                <p
                  style={{
                    color: '#718096',
